...
const CreateScore = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (![clarity, completeness, correctness].every(isNumberInRange)) {
    throw CustomError(
      400,
      "clarity, completeness, correctness must be numbers 1-10",
    );
  }
  ...
  return res
    .status(201)
    .json(new APIResponse("Score created successfully", { score }));
});
...
const GetScore = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json(new APIResponse("Score fetched successfully", { score }));
});
